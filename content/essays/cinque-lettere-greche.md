...
C'è una seconda lettura di questa somma, e viene dallo schema in quattro tappe che governa ogni tipo — **FIEC**: Formazione, Introduzione, Eliminazione, Computazione, la mappa di [*Una promessa*](../una-promessa). La quarta tappa, la **C**, è sempre la stessa mossa: l'eliminatore che incontra un introduttore e calcola. Ma non è una lettera sola. Per il tipo funzione, l'incontro tra applicazione e λ è β; per ogni tipo induttivo, l'incontro tra ricorsore e costruttore è ι. β e ι sono dunque la *stessa* tappa dello schema — la Computazione — istanziata su due famiglie: una primitiva, una schematica. È per questo che si somigliano pur sembrando estranee: β è il motore della funzione, ι il motore dell'induzione, ma il gesto è uno. η, accanto, non è una C: è il suo duale, la regola di *unicità* che certe presentazioni aggiungono al type former — non «come si usa un introduttore», ma «ogni abitante *è* un introduttore». E δ e ζ, da questo punto di vista, sono le uniche due lettere che non appartengono allo schema di nessun tipo: non dicono nulla di Formazione, Introduzione, Eliminazione o Computazione — sono la contabilità dei nomi, e basta. Le cinque lettere, ricontate per ruolo, sono due Computazioni, una unicità, due regole del nominare.

E cinque è il conto del *CIC*, non di ogni teoria à la Martin-Löf. La MLTT intensionale originaria adopera le due Computazioni e δ; ζ serve solo dove c'è un `let` primitivo, e η — l'abbiamo visto nella sua sezione — semplicemente non c'era: l'uguaglianza delle funzioni restava per intero dal lato proposizionale, ed è scivolata nella conversione di Agda e di Coq solo più tardi, lasciando ancora oggi i sistemi in disaccordo. Le cinque lettere non sono un dato della teoria dei tipi: sono il bilancio di un calcolo particolare, il punto cui sono arrivate le sue implementazioni mature.
